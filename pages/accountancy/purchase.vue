...
.formWrapper
  ...
  .enrollmentForm
    ...
    .infoLine
        label
            | Veli Ad {{parentName}}
        input(type="text" placeholder="İsim" v-model="parentName")
    .infoLine
        label
            | Veli Soyad {{parentSurname}}
        input(type="text" placeholder="Soyad" v-model="parentSurname")
    .infoLine(v-show="newParent")
        .infoLine
          input(type="submit" value="Veli Ekle" @click="enroll('parent')")
    .infoLine
        label
            | Öğrenci
    ...
    .infoLine
        label
            | Öğrenci Adı
        input(type="text" placeholder="İsim" v-model="studentName")
    .infoLine
        label
            | Öğrenci Soyad 
        input(type="text" placeholder="Soyad" v-model="studentSurname")
    .infoLine
        label
            | Doğum Tarihi {{studentBirthday}}
        input(type="date" v-model="studentBirthday")
    .infoLine
        label
            | Sınıfı
        select(v-model="mygrade" )
            option Sınıf
            option(v-for="grade in grade()" :value="grade._id") {{grade.gradeName}}
    .infoLine(v-show="newStudent")
        .infoLine
          input(type="submit" value="Öğrenci Ekle" @click="enroll('student')")
    .infoLine
        label(v-if="packageId != ''")
            | {{package()[packageId].name}}
        label(v-if="packageId == ''")
...
export default {
  ...
  methods: {
    ...mapActions("users", ["addUser", "getUsers"]),
    ...
    enroll: async function(type) {
      let newUser = {};
      if (type == "parent") {
        if (this.parentPhone != "") newUser.phone = this.parentPhone;
        if (this.parentSurname != "") newUser.surname = this.parentSurname;
        if (this.parentName != "") newUser.name = this.parentName;
        this.parentPhone = "0"
        await this.addUser(newUser);
        await this.getUsers()
        this.parentPhone = newUser.phone
      } else if (type == "student") {
        if (this.studentName != "") newUser.name = this.studentName;
        if (this.studentSurname != "") newUser.surname = this.studentSurname;
        if (this.studentPhone != "") newUser.phone = this.studentPhone;
        if (this.studentBirthday != "") newUser.birthDay = this.studentBirthday;
        if (this.mygrade != "") newUser.grade = this.mygrade;
        this.studentPhone = "0"
        await this.addUser(newUser);
        await this.getUsers()
        this.studentPhone = newUser.phone
      }
    },
    apply: function() {
      let branches = [];
    ...
    addPurchase: async function(purchase) {
      await this.$axios
        ...
        .then(res => {
          if (res.status == 201) {
            this.$router.push("/accountancy/purchases");
          }
        });
    },
            ...
            .then(res => {
              if (res.data.user == null) {
                ...
                if (
                  ...
                  this.parentName = res.data.user.name;
                  this.parentSurname = res.data.user.surname;
            ...
            .then(res => {
              if (res.data.user != null) {
                ...
                if (
                  ...
                  this.studentSurname = res.data.user.surname;
                  this.mygrade = res.data.user.grade;
                  if (res.data.user.birthDay != undefined) {
